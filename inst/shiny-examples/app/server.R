...
shinyServer(function(input, output) {
  ...
  output$hwe_plot <- renderPlot({
    ...
    ## This is taken from the RcolorBrewer Set1
    col.scheme <- c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00")
    ...
    for (i in seq(data_set$r)) {
      lines(x = data_set$results$gen, y = data_set$results[,i+1], col = col.scheme[i], lwd = 2)
    }
  ...
  output$var_prop <- renderPlot({
    ...
    bp <- barplot(height = var_out$V_G_prop, horiz = TRUE, beside = FALSE,
                  ...
                  xlab = expression(Proportion~of~V[G]), col = c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00")[1:2],
                  legend.text = T)
  ...
  output$var_plot <- renderPlot({
    ...
    colors <- c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00")[1:2]
  ...
  output$resp_plot <- renderPlot({
    ...
    ## Add lines
    lines(x = resp_out$results$gen, y = resp_out$results$G, lwd = 2.5, col = "#377EB8")
    ...
    ## Add lines
    lines(x = resp_out$results$gen, y = resp_out$results$varG, lwd = 2.5, col = "#E41A1C")
